...
PYBIND11_MODULE(binding_test, m) {
    ...
    auto surfaceInfoInit = [](int logicalWidth,
                              int logicalHeight,
                              int physicalWidth,
                              int physicalHeight,
                              float contentScaleX,
    ...
    py::class_<SurfaceInfo>(m, "SurfaceInfo").def(py::init(surfaceInfoInit));

    py::class_<Engine>(m, "Engine")
        .def(py::init<const AbstractReader&>())
        .def("render", [](Engine& engine, float playbackValue) { engine.render(playbackValue); })
    ...
    py::class_<AssimpReader, AbstractReader>(m, "AssimpReader").def(py::init<std::string_view>());

    py::class_<SketchupReader, AbstractReader>(m, "SketchupReader").def(py::init<std::string_view>());
